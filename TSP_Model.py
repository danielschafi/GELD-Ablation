...
class TSPModel(nn.Module):
    ...
    def pre_forward(
        self, state, Norm=True, exact_distance_matrix=False, compute_region_map=True
    ):
        ...
        map the coordinates to regions
    ...
    def forward(
        ...
        if self.mode == "test" or not self.training:
            if repair == False:
                ...
                if not beam_search:
                    probs = self.decoder(
                        self.encoded_nodes, selected_node_list, self.dis_matrix
                    )
                    selected_student = probs.argmax(dim=1)
...
class TSP_Encoder(nn.Module):
    ...
    Enriched representation of node embeddings"""
    ...
    def __init__(self, **model_params):
        ...
        self.layers_global = nn.ModuleList(
            [EncoderLayer(**model_params) for _ in range(1)]
        )
...
class TSP_Decoder(nn.Module):
    ...
    On the local K_set around last node, predicting logits of the probabilities of selecting one of the nodes in K_set
    """
    ...
    def _get_new_data(
        ...
        if self.training:
            ...
            if new_list_len <= num_k:
                unselect_list = new_list[torch.gt(new_list, -1)].view(
                    batch_size, new_list_len
                )
            else:
                last_node = selected_node_list[:, -1]
                if beam_search:
                    last_node = last_node.view(B_V, beam_size, 1).expand(
                        -1, -1, prob_size
                    )
                    distance = dis_matrix.gather(1, last_node).view(B_V * beam_size, -1)
                else:
                    distance = dis_matrix.gather(
                        1, last_node.view(batch_size, 1, 1).expand(-1, -1, prob_size)
                    ).squeeze(1)
                mask = torch.zeros_like(distance)
                mask[index_1, selected_node_list] = 1e2
                unselect_list = torch.topk(
                    distance + mask, dim=1, k=num_k, largest=False
                ).indices
        ...
        if beam_search:
            ...
            data_global = torch.gather(
                new_data, 1, unselect_list.unsqueeze(2).expand(-1, -1, emb_dim)
            )

        return data_global, unselect_list

    def _get_encoding(
        self, encoded_nodes, node_index_to_pick, beam_search=False, beam_size=16
    ):
        ...
        if beam_search:
            encoded_nodes = encoded_nodes.unsqueeze(1).expand(
                batch_size // beam_size, beam_size, -1, embedding_dim
            )
            gathering_index = node_index_to_pick.view(
                batch_size // beam_size, beam_size, pomo_size, 1
            ).expand(batch_size // beam_size, beam_size, pomo_size, embedding_dim)
    ...
    def forward(
        self, data, selected_node_list, dis_matrix, beam_search=False, beam_size=16
    ):
        ...
        global_info, unselect_list = self._get_new_data(
            new_data,
            selected_node_list,
            problem_size,
            batch_size_V,
        ...
        out_global = torch.cat(
            (
                embedded_first_node_1.unsqueeze(1),
                global_info,
                embedded_last_node_1.unsqueeze(1),
            ),
            dim=1,
        )
        unselect_list = torch.cat(
            (
                selected_node_list[:, 0].unsqueeze(1),
                unselect_list,
                selected_node_list[:, -1].unsqueeze(1),
            ),
            dim=-1,
        ).type(torch.long)
        ...
        if problem_size > 10000:
            ...
            if beam_search:
                index_un = unselect_list.view(batch_size_V, beam_size, -1, 1).expand(
                    batch_size_V, beam_size, -1, 2
                )
                cor = (
                    self.data.view(batch_size_V, 1, -1, 2)
                    .expand(batch_size_V, beam_size, -1, 2)
                    .gather(dim=2, index=index_un)
                    .view(batch_size_V * beam_size, -1, 2)
                )
            ...
            if beam_search:
                index_1_ = (
                    torch.arange(batch_size_V, dtype=torch.long)
                    .unsqueeze(1)
                    .unsqueeze(2)
                    .unsqueeze(3)
                )
                index_2_ = (
                    torch.arange(beam_size, dtype=torch.long)
                    .unsqueeze(0)
                    .unsqueeze(2)
                    .unsqueeze(3)
                )
                index_un = unselect_list.view(batch_size_V, beam_size, -1, 1)
                dis_matrix = dis_matrix.unsqueeze(1).expand(
                    batch_size_V, beam_size, problem_size, problem_size
                )
                dis_matrix_un = dis_matrix[
                    index_1_, index_2_, index_un, index_un.transpose(2, 3)
                ].view(
                    batch_size_V * beam_size,
                    unselect_list.shape[-1],
                    unselect_list.shape[-1],
                )
            else:
                index_1_ = (
...
class EncoderLayer(nn.Module):
    ...
    With Region aware linear attention RALA"""
    ...
    def forward(self, input1, index_region):
        ...
        multi_head_out = self.multi_head_combine(
            self.attentionlayer(hidden_states, index_region)
        )
...
class DecoderLayer(nn.Module):
    ...
    Also uses Attention but the one by Zhou et al., 2024"""
    ...
    def __init__(self, **model_params):
        ...
        self.multi_head_combine = nn.Linear(
            head_num * qkv_dim, embedding_dim, bias=False
        )
    ...
    def forward(self, input1, dis_matrix):
        ...
        multi_head_out = self.multi_head_combine(
            self.attentionlayer(hidden_states, dis_matrix)
        )
        out1 = input1 + multi_head_out
...
class SparseAttention(nn.Module):
    ...
    def forward(self, x, index_region):
        ...
        region_mask.scatter_add_(
            dim=1, index=index_region, src=torch.ones(index_region.shape)
        )
        ...
        region_sums.scatter_add_(
            dim=1,
            index=index_region.unsqueeze(-1).expand(agent_matrix.shape),
            src=agent_matrix,
        )
