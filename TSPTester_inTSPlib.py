...
from Test_data.test import tsplib_collections, national_collections
...
class TSPTester:
    ...
    def run(self):
        ...
        TSPLIB_or_Nation = False
        if TSPLIB_or_Nation:
            # for TSPLIB
            tsplib_names = list(tsplib_collections.keys())
        else:
            # for nationalTSP
            tsplib_names = list(national_collections.keys())
        ...
        for i in range(len(tsplib_names)):
            ...
            if TSPLIB_or_Nation:
                ...
                score_teacher, score_student, problems_size, tour = (
                    self._test_one_batch(
                        episode,
                        1,
                        clock=self.time_estimator_2,
                        Name=name,
                        TSPLIB_or_Nation=TSPLIB_or_Nation,
            ...
            # save soltions
            GELD_solution[name.upper()] = tour.cpu()
            ...
            print(
                "problems_100_500 mean gap:",
                np.mean(problems_100_500),
                len(problems_100_500),
            )
            print(
                "problems_500_1k mean gap:",
                np.mean(problems_500_1k),
                len(problems_500_1k),
            )
            print(
                "problems_1k_5k mean gap:", np.mean(problems_1k_5k), len(problems_1k_5k)
            )
            print(
                "problems_5k_10k mean gap:",
                np.mean(problems_5k_10k),
                len(problems_5k_10k),
            )
            ...
            if all_done:
                if not self.env_params["test_in_tsplib"]:
                    ...
                    self.logger.info(
                        " Student SCORE: {:.4f} ".format(score_student_AM.avg)
                    )
                    self.logger.info(
                        " Gap: {:.4f}%".format(
                    ...
                    gap_ = average_gap
        # np.save('GELD_so.npy', GELD_solution)
    ...
    def _test_one_batch(
        ...
        with torch.no_grad():
            ...
            self.logger.info(
                "greedy, name:{}, gap:{:5f} %, stu_l:{:5f} , opt_l:{:5f}".format(
                    name,
                    gap,
            ...
            if self.tester_params["beam"]:
                ...
                current_best_length_beam, min_idx = reward.min(1)
                zero_to_bsz = torch.arange(
                    batch_size, dtype=torch.long, device=current_best_length_beam.device
                )
                best_select_node_list_beam = selected_node_list[zero_to_bsz, min_idx]
                if current_best_length_beam < current_best_length:
                    current_best_length = current_best_length_beam
                    best_select_node_list = best_select_node_list_beam
                gap = (
                    (current_best_length.mean() - self.optimal_length.mean())
                    / self.optimal_length.mean()
                ).item() * 100
                self.logger.info(
                    "beam, name:{}, gap:{:5f} %,  stu_l:{:5f} , opt_l:{:5f}".format(
                        name,
                        gap,
                        current_best_length.mean().item(),
                        self.optimal_length.mean().item(),
            ...
            # plot
            # self.env.drawPic(self.origin_problem.squeeze(0), best_select_node_list.squeeze(0))
            self.logger.info(
                "PRC, name:{}, gap:{:5f} %,  Elapsed[{}], stu_l:{:5f} , opt_l:{:5f}".format(
                    name,
                    gap,
                    escape_time,
                    current_best_length.mean().item(),
