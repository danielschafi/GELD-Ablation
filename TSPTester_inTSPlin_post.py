...
from Test_data.test import national_collections
...
class TSPTester:
    ...
    def run(self):
        ...
        ss_solutions = np.load(
            "baseline_solutions/INV_so.npy", allow_pickle=True
        ).item()
    ...
    def _test_one_batch(
        self, episode, batch_size, clock=None, Name=None, opt_len=None, solution=None
    ):
        ...
        with torch.no_grad():
            ...
            best_select_node_list = torch.tensor(
                solution, device=self.origin_problem.device
            ).unsqueeze(0)
            ...
            if self.tester_params["PRC"]:
                ...
                for step_RC in range(num_RC):
                    val_num_samples = torch.randint(
                        low=lower, high=sample_max + 1, size=[1]
                    )[0]
                    ...
                    if problem_size <= 100000:
                        ...
                        len_of_sub = torch.randint(
                            low=4, high=min(high_max, max_lenth) + 1, size=[1]
                        )[0]
