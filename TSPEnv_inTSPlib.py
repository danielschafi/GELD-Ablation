...
class TSPEnv:
    ...
    def load_problems(
        self, episode, batch_size, Name=None, opt_len=None, TSPLIB_or_Nation=False
    ):
        ...
        if not self.test_in_tsplib:
            ...
            self.problems, _ = load_tsplib_file(
                root=Path("Test_data/"),
                tsplib_name=Name,
        ...
        if mode == "test":
            ...
            if length_fix:
                ...
                length_of_subpath = torch.randint(
                    low=4, high=problems_size + 1, size=[1]
                )[0]  # in [4,N]
                # length_of_subpath = problems_size
    ...
    def make_dataset(self, filename, episode, batch_size, num_samples):
        ...
        tour = []
        for line in tqdm(
            open(filename, "r").readlines()[episode : episode + batch_size], ascii=True
        ):
            line = line.split(" ")
            num_nodes = int(line.index("output") // 2)
            nodes_coords.append(
