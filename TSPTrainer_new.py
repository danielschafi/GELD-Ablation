...
class TSPTrainer:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.trainer_params["epochs"] + 1):
            ...
            # Train
            train_score, train_student_score, train_loss = self._train_one_epoch(
                epoch, problem_size=problem_size
            )
    ...
    def _train_one_epoch(self, epoch, problem_size=100):
        ...
        episode = 0
        loop_cnt = 0
        self.env.load_problems_4_each_epoch(
        ...
        while (
            iterations < max_limit
            and (greedy_values.mean() - best_values) / best_values > 0.001
            and best_limit > 0
        ):
            if best_values > PRC_value:
                ...
                best_limit -= 1
            self.logger.info(
                "greedy_values: {:3f}, best_values: {:.3f}".format(
                    greedy_values.mean(), best_values
                )
            )
            self.logger.info(
                "iteration: {:2d}, gap: {:.3f}".format(
                    iterations, (greedy_values.mean() - best_values) / best_values
                )
            )
    ...
    def PRC(self, all_list, num_RC=1000):
        ...
        for step_RC in range(num_RC):
            ...
            while episode < val_num_episode:
                ...
                all_node_list = (
                    torch.cat((all_node_list, after_repair_complete_solution), dim=0)
                    if all_node_list is not None
                    else after_repair_complete_solution
                )
                current_best_length = self.env._get_travel_distance_2(
                    origin_problem, after_repair_complete_solution
                )
                all_value = (
                    torch.cat((all_value, current_best_length), dim=0)
                    if all_value is not None
                    else current_best_length
                )
                episode += batch_size
    ...
    def validation_beam(self, problem_size):
        ...
        while episode < val_num_episode:
            remaining = val_num_episode - episode
            batch_size = min(self.trainer_params["val_beam_batch_size"], remaining)
            initial_value, initial_list = self.beamsearch_tour_nodes_shortest(
                beam_size,
                batch_size,
                problem_size,
                self.dtypeFloat,
            ...
            all_value = (
                torch.cat((all_value, initial_value), dim=0)
                if all_value is not None
                else initial_value
            )
            all_node_list = (
                torch.cat((all_node_list, initial_list), dim=0)
                if all_node_list is not None
                else initial_list
            )
            episode += batch_size
    ...
    def beamsearch_tour_nodes_shortest(
        ...
        zero_to_bsz = torch.arange(
            batch_size, dtype=torch.long, device=min_values.device
        )
    ...
    def validation_greedy(self):
        ...
        with torch.no_grad():
            while episode < val_num_episode:
                ...
                initial_value, initial_list = self._val_batch_greedy(
                    episode, batch_size
                )
                all_value = (
                    torch.cat((all_value, initial_value), dim=0)
    ...
    def _val_batch_greedy(self, episode, batch_size):
        ...
        current_best_length = self.env._get_travel_distance_2(
            self.env.problems, best_select_node_list
        )
