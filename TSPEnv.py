...
class TSPEnv:
    ...
    def load_problems(self, episode, batch_size, mix=False, train=False):
        ...
        if mix:
            ...
            problems_small, solution_small = (
                self.raw_data_nodes_100[index],
                self.raw_data_tours_100[index],
            )
            problems_large, solution_large = (
                self.raw_data_nodes[episode : episode + batch_size],
                self.raw_data_tours[episode : episode + batch_size],
            )
            if self.sub_path:
                problems_large, solution_large = self.sampling_subpaths(
            ...
            index_bsz = torch.arange(
                batch_size, dtype=torch.long, device=problems_large.device
            )
            cor = (
                problems_small[index_bsz, solution_small[:, 0]]
                .unsqueeze(1)
                .repeat(1, node_gap, 1)
            )
            ...
            # Take batch size num problems and solutions
            # self.raw_data_nodes = Node coordinates  / x / input
            # self.raw_data_tours = baseline solutions  / y / labels
            ...
            # with p=0.5 invert the solution tour
            if_inverse = True
            if_inverse_index = torch.randint(low=0, high=100, size=[1])[0]
            if if_inverse_index < 50:
                if_inverse = False
    ...
    def sampling_subpaths(
        ...
        first_node_index = torch.randint(low=0, high=problems_size, size=[1])[
            0
        ]  # in [0,N)
        if mode == "test":
            length_of_subpath = torch.randint(
                low=low_index, high=problems_size + 1, size=[1]
            )[0]  # in [4,N]
        else:
            if length_fix:
        ...
        index_2, _ = (
            torch.cat((new_sulution_ascending, new_sulution_ascending), dim=1)
            .type(torch.long)
    ...
    def load_raw_data(
        ...
        if SL_Test:
            if MSV:
                root = "./Test_data/"
                instances, b_tours, _ = load_instances_with_baselines(
                    root, size, disribution
                )
                ...
                for line in tqdm(
                    open(self.data_path, "r").readlines()[
                        0 + begin_index : episode + begin_index
                    ],
                    ascii=True,
                ):
                    line = line.split(" ")
                ...
                self.raw_data_tours = torch.tensor(
                    self.raw_data_tours, requires_grad=False
                )
                print(
            ...
            self.raw_data_nodes_100 = torch.tensor(
                self.raw_data_nodes_100, requires_grad=False
            )
            self.raw_data_tours_100 = torch.tensor(
                self.raw_data_tours_100, requires_grad=False
            )
            print(
                "load raw dataset done!",
            )
    ...
    def reset(self, batch_size=None):
        ...
        loads the problems,
        returns a copy of the problems"""
    ...
    def pre_step(self):
        """Resets rewards but keeps state

        returns the envs internal step state"""
        reward = None
        reward_student = None
        done = False
    ...
    def step(self, selected, selected_student):
        ...
        # append selected node to list of nodes till now
        ...
        if done:
            reward_student = self._get_travel_distance_2(
                self.problems, self.selected_student_list
            )
    ...
    def _get_travel_distance(self):
        if self.solution is not None:
            ...
            ordered_seq_student = self.problems.gather(
                dim=1, index=gathering_index_student
            )
    ...
    def get_travel_distance_multi(self, selected_node_list):
        batch_size, problem_size = (
            selected_node_list.size(0),
            selected_node_list.size(2),
        )
        gathering_index = selected_node_list.unsqueeze(3).expand(
            batch_size, -1, problem_size, 2
        )
        # shape: (batch, num_multi, problem, 2)
        seq_expanded = self.problems[:, None, :, :].expand(
            batch_size, selected_node_list.size(1), problem_size, 2
        )
    ...
    def _get_travel_distance_2(self, problems, solution):
        """Return travel distances for a batch of problems/solutions"""
