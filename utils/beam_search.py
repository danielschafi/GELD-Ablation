...
class Beamsearch(object):
    ...
    def advance(self, trans_probs, selected_nodes):
        ...
        if len(self.prev_Ks) > 0:
            ...
            if self.probs_type == "raw":
                beam_lk[:, 1:] = torch.zeros(
                    beam_lk[:, 1:].size(), device=self.device
                ).type(self.dtypeFloat)
            elif self.probs_type == "logits":
                beam_lk[:, 1:] = -1e20 * torch.ones(
                    beam_lk[:, 1:].size(), device=self.device
                ).type(self.dtypeFloat)
    ...
    def get_hypothesis(self, k):
        ...
        hyp = -1 * torch.ones(self.batch_size, self.num_nodes, device=self.device).type(
            self.dtypeLong
        )
